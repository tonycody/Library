...
{
    unsafe class ManagedReedSolomon8 : IThisLock
    {
        private volatile ManagedReedSolomon8.Math _fecMath;
        ...
        public ManagedReedSolomon8(int k, int n, int threadCount, BufferManager bufferManager)
        {
            _fecMath = new Math();
            _k = k;
            _n = n;
        ...
        private void Encode(byte[][] src, int[] srcOff, byte[][] repair, int[] repairOff, int[] index, int packetLength)
        {
            Parallel.For(0, repair.Length, new ParallelOptions() { MaxDegreeOfParallelism = _threadCount }, row =>
            {
                ...
                else
                {
                    ...
                    fixed (byte* p_pkt = repair[row])
                    {
                        for (int col = 0; col < _k; col++)
                        {
                            if (_cancel) return;

                            _fecMath.AddMul(p_pkt, repairOff[row], src[col], srcOff[col], (byte)_encMatrix[pos + col], packetLength);
                        }
                    }
                }
            });
        ...
        public void Decode(ArraySegment<byte>[] pkts, int[] index, int size)
        {
            ...
            lock (this.ThisLock)
            {
                ManagedReedSolomon8.Shuffle(pkts, index, _k);
        ...
        private void Decode(byte[][] pkts, int[] pktsOff, int[] index, int packetLength)
        {
            ...
            Parallel.For(0, _k, new ParallelOptions() { MaxDegreeOfParallelism = _threadCount }, row =>
            {
                ...
                if (index[row] >= _k)
                {
                    ...
                    fixed (byte* p_pkt = tmpPkts[row])
                    {
                        for (int col = 0; col < _k; col++)
                        {
                            if (_cancel) return;

                            _fecMath.AddMul(p_pkt, 0, pkts[col], pktsOff[col], (byte)decMatrix[row * _k + col], packetLength);
                        }
                    }
                }
            });
        ...
        private unsafe class Math
        {
            ...
            public void AddMul(byte* dst, int dstPos, byte[] src, int srcPos, byte c, int len)
            {
                ...
                int unroll = 8; // unroll the loop 8 times.
                int i = dstPos;
                int j = srcPos;
                int lim = dstPos + len;
                ...
                {
                    {
                        ...
                        for (; i < lim && (lim - i) > unroll; i += unroll, j += unroll)
                        {
                            ...
                            buffer[0] = p_gf_mulc[p_src[j]];
                            buffer[1] = p_gf_mulc[p_src[j + 1]];
                            buffer[2] = p_gf_mulc[p_src[j + 2]];
                            buffer[3] = p_gf_mulc[p_src[j + 3]];
                            buffer[4] = p_gf_mulc[p_src[j + 4]];
                            buffer[5] = p_gf_mulc[p_src[j + 5]];
                            buffer[6] = p_gf_mulc[p_src[j + 6]];
                            buffer[7] = p_gf_mulc[p_src[j + 7]];

                            *((long*)(dst + i)) ^= *((long*)buffer);
                        }
                    }
                        ...
                        fixed (byte* p_src = src)
                        {
                            for (int p = 0, ix = 0; ix < k; ix++, p += k)
                            {
                                if (ix != icol)
                                {
                                    c = src[p + icol];
                                    src[p + icol] = (byte)0;
                                    AddMul(p_src, p, src, pivotRowPos, c, k);
                                }
                            }
                        }
                    }
