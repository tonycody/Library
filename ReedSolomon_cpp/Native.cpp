...
ReedSolomon::Math::Math(){};
ReedSolomon::Math::Math(int gfBits){
    _gfBits = gfBits;
    _gfSize = ((1 << gfBits) - 1);
...
void ReedSolomon::Math::GenerateGF(){
...
void ReedSolomon::Math::InitMulTable(){
    if (_gfBits <= 8)
    {
...
uint8_t ReedSolomon::Math::Modnn(int x){
    while (x >= _gfSize)
    {
        x -= _gfSize;
...
uint8_t ReedSolomon::Math::Mul(uint8_t x, uint8_t y){
    if (_gfBits <= 8)
    {
...
uint8_t* ReedSolomon::Math::CreateGFMatrix(int rows, int cols){
	return new uint8_t[rows * cols];
};
void ReedSolomon::Math::AddMul(uint8_t* dst, int dstPos, uint8_t* src, int srcPos, uint8_t c, int len){
    if (c == 0)
    {
        return;
    }
...
void ReedSolomon::Math::MatMul(uint8_t* a, int aStart, uint8_t* b, int bStart, uint8_t* c, int cStart, int n, int k, int m){
...
void ReedSolomon::Math::InvertMatrix(uint8_t* src, int k){
	int* indxc = new int[k];
	int* indxr = new int[k];
...
void ReedSolomon::Math::InvertVandermonde(uint8_t* src, int k){
        if (k == 1)
        {
            return;
        }
...
uint8_t* ReedSolomon::Math::CreateEncodeMatrix(int k, int n){
...
uint8_t* ReedSolomon::Math::CreateDecodeMatrix(uint8_t* encMatrix, int* index, int k, int n){
...
bool ReedSolomon::Math::Equals(uint8_t* sourceCollection, int sourceIndex, uint8_t* destinationCollection, int destinationIndex, int length){
...
void ReedSolomon::CopyShuffle(uint8_t** pkts, int32_t* index, int k){
    uint8_t* buffer = NULL;
...
ReedSolomon::ReedSolomon(int gfBits, int k, int n, int threadCount){
...
void ReedSolomon::Encode(uint8_t** src, int32_t* srcOffset, uint8_t** repair, int32_t* repairOffset, int32_t* index){
	Encode(src, srcOffset, repair, repairOffset, index, sizeof(src[0]));
};
void ReedSolomon::Decode(uint8_t** pkts, int32_t* pktsOffset, int* index){
	CopyShuffle(pkts, index, _k);
	Decode(pkts, pktsOffset, index, sizeof(pkts[0]));
};
void ReedSolomon::Encode(uint8_t** src, int32_t* srcOff, uint8_t **repair, int32_t* repairOff, int32_t* index, int packetLength){
...
void ReedSolomon::Decode(uint8_t** pkts, int32_t* pktsOff, int32_t* index, int packetLength){
