...
{
    [DataContract(Name = "WhisperMessage", Namespace = "http://Library/Net/Lair")]
    public sealed class WhisperMessage : ReadOnlyCertificateItemBase<WhisperMessage>, IWhisperMessage<Whisper, Key>
    {
        ...
        private Whisper _chat = null;
        private DateTime _creationTime = DateTime.MinValue;
        private Key _content = null;

        private Certificate _certificate;

        public WhisperMessage(Whisper chat, Key content, DigitalSignature digitalSignature)
        {
            this.Whisper = chat;
        ...
        protected override void ProtectedImport(Stream stream, BufferManager bufferManager)
        {
            ...
            for (; ; )
            {
                ...
                {
                    if (id == (byte)SerializeId.Whisper)
                    {
                        this.Whisper = Whisper.Import(rangeStream, bufferManager);
                    }
        ...
        public override Stream Export(BufferManager bufferManager)
        {
            ...
            {
                Stream exportStream = this.Whisper.Export(bufferManager);

                BufferStream bufferStream = new BufferStream(bufferManager);
                bufferStream.Write(NetworkConverter.GetBytes((int)exportStream.Length), 0, 4);
                bufferStream.WriteByte((byte)SerializeId.Whisper);
        ...
        {
            if ((object)obj == null || !(obj is WhisperMessage)) return false;

            return this.Equals((WhisperMessage)obj);
        }

        public override bool Equals(WhisperMessage other)
        {
        ...
        public override WhisperMessage DeepClone()
        {
            using (var stream = this.Export(BufferManager.Instance))
            {
                return WhisperMessage.Import(stream, BufferManager.Instance);
            }
        }
