...
namespace Library.Net.Lair
{
    ...
    public sealed class Profile : ReadOnlyCertificateItemBase<Profile>, IProfile<Section>
    {
        ...
        public Profile(Section section, string comment, SignatureCollection trustSignatures, ChannelCollection channels, DigitalSignature digitalSignature)
        {
        ...
        protected override void ProtectedImport(Stream stream, BufferManager bufferProfile)
        {
            ...
            for (; ; )
            {
                ...
                using (RangeStream rangeStream = new RangeStream(stream, stream.Position, length, true))
                {
                    ...
                    {
                        this.Section = Section.Import(rangeStream, bufferProfile);
                    }
                    ...
                    {
                        this.ProtectedChannels.Add(Channel.Import(rangeStream, bufferProfile));
                    }

                    else if (id == (byte)SerializeId.Certificate)
                    {
                        this.Certificate = Certificate.Import(rangeStream, bufferProfile);
                    }
                }
            }
        }

        public override Stream Export(BufferManager bufferProfile)
        {
            ...
            {
                Stream exportStream = this.Section.Export(bufferProfile);

                BufferStream bufferStream = new BufferStream(bufferProfile);
            ...
            {
                Stream exportStream = c.Export(bufferProfile);
            ...
            {
                Stream exportStream = this.Certificate.Export(bufferProfile);
