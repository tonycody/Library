...
namespace ReedSolomon_cs
{
    ...
    class Program
    {
        ...
        {
            ReedSolomon pc = new ReedSolomon(8, 128, 256, 1);
            ...
            using (StreamWriter sw = new StreamWriter("cs.txt"))
            {
                for (int i = 0; i < buffList.Count; i++)
                {
                    for (int j = 0; j < buffList[i].Count; j++)
                    {
                        sw.Write(buffList[i].Array[j].ToString() + " ");
                    }
                }
                sw.WriteLine();
                for (int i = 0; i < buffList.Count; i++)
                {
                    sw.Write(buffList[i].Offset.ToString() + " ");
                }
                sw.WriteLine();
                for (int i = 0; i < buffList2.Count; i++)
                {
                    for (int j = 0; j < buffList2[i].Count; j++)
                    {
                        sw.Write(buffList2[i].Array[j].ToString() + " ");
                    }
                }
                sw.WriteLine();
                for (int i = 0; i < buffList2.Count; i++)
                {
                    sw.Write(buffList2[i].Offset.ToString() + " ");
                }
                sw.WriteLine();
                for (int i = 0; i < buffList3.Count; i++)
                {
                    for (int j = 0; j < buffList3[i].Count; j++)
                    {
                        sw.Write(buffList3[i].Array[j].ToString() + " ");
                    }
                }
                sw.WriteLine();
                for (int i = 0; i < buffList3.Count; i++)
                {
                    sw.Write(buffList3[i].Offset.ToString() + " ");
                }
                sw.WriteLine();
            }
        }
